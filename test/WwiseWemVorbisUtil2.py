...
class WemFormatChunk:
    fmt_id = None  # 4字节
    fmt_size = None  # 4字节 数据字段包含数据的大小。如无扩展块，则值为16；有扩展块，则值为= 16 + 2字节扩展块长度 + 扩展块长度或者值为18（只有扩展块的长度为2字节，值为0）
    ...
    fmt_samples_per_sec = None  # 4字节 音频的码率，每秒播放的字节数。samples_per_sec * channels * bits_per_sample / 8，可以估算出使用缓冲区的大小
    fmt_avg_bytes_per_sec = None  # 4字节
    fmt_blockalign = None  # 2字节
    fmt_bits_per_sample = None  # 2字节
    fmt_cb_size = None  # 2字节 扩展区长度
...
def main():
    ...
    sub_format = test_wem_file.read(48)  # GUID，include the data format code，数据格式码。
    # Todo 卡在这里了，这48个字节到底是干嘛用的？？？？
    print("sub_format: " + str(sub_format))
