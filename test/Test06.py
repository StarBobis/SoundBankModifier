...
def make_window():
    ...
    table_def = [[]]
    layout = [
        [sg.Menu(menu_def, key='menu')], [sg.Table(table_def, headings=["Id", "Offset", "Length (Bytes)", "Replacing with"], key="_table_", expand_x=True,expand_y=True)]
    ]
...
def main():
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED:
            ...
            output_wem_folderPath = output_wem_folder + "/"

            if bnk_info == None:
                if output_wem_folder != "":
                    sg.popup_error("请先打开一个bnk文件")
            else:
                DIDX = bnk_info.DIDX_Section
                save_all_wem(DIDX.WEMInfos, output_wem_folderPath)
                sg.popup("导出成功！")
